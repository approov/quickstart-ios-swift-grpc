...
# BITCODE SUPPORT
The Approov SDK is also available with bitcode support. If you wish to use it then you should modify the branch for the Approov service layer dependency to `2.9.0-bitcode`. Please then also use the `-bitcode` option when registering your application with the Approov service.
...
The default header name of `Approov-Token` can be changed by setting the variable `ApproovService.approovTokenHeaderAndPrefix` like so:
...
An Approov app automatically downloads any new configurations of APIs and their pins that are available. These are stored in the [`UserDefaults`](https://developer.apple.com/documentation/foundation/userdefaults) for the app in a preference key `approov-dynamic`. You can store the preferences differently by modifying or overriding the methods `storeDynamicConfig` and `readDynamicApproovConfig` in `ApproovService.swift`.
