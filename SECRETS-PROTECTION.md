...
With this in place, network calls using an `ApproovClientInterceptor` will replace `your-secret-name` with `your-secret-value` as required when the app passes attestation. Since the mapping lookup is performed on the secret name you have the flexibility of providing different secrets on different API calls, even if they are passed using the same header name.
